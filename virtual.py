"""
Virtual Air Pen with Thumbs-Up Gesture Control
File: virtual_air_pen.py
Author: ChatGPT (GPT-5 Thinking mini)

Description:
- Uses MediaPipe Hands + OpenCV to track two hands in realtime.
- One hand (pen hand) draws using index-finger tip as stylus.
- The other hand's "thumbs-up" gesture toggles drawing mode on.
- Optional GPU acceleration: uses PyTorch (if available) to perform
  smoothing/filtering of landmark coordinates on GPU. Also attempts
  to use OpenCV CUDA for simple image upload when available.

Requirements:
- Python 3.8+
- pip install opencv-python mediapipe torch torchvision numpy
  (torch is optional but recommended for GPU acceleration)

Run:
python virtual_air_pen.py --camera 0 --width 1280 --height 720 --use-gpu

Notes on GPU:
- MediaPipe's Python API does not automatically use GPU in standard pip installs.
  This script does not change MediaPipe internals; instead it offloads
  coordinate smoothing and tensor operations to CUDA via PyTorch when
  --use-gpu is specified and a CUDA device is available.
- OpenCV CUDA functions are attempted when available but are optional.
...
def main(args):
    ...
    last_two_finger_state = False  # track last state to detect toggles
    ...
    try:
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                # hand temporarily lost, keep current drawing mode
                if time.time() - last_seen_time > hand_lost_timeout:
                    pen_point = None

                # reset toggle detection so next 2 fingers triggers again
            ...
            cv2.imshow("Virtual Air Pen (Single Hand Mode)", overlay)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Virtual Air Pen (Single Hand)')
